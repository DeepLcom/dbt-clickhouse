...
class TestRemoteTableRemoteConfig:
    ...
    def init_local_table(self, project):
        project.run_sql(
            f"create database if not exists _{project.test_schema} on cluster test_shard"
        )
        project.run_sql(
            f"""
    ...
    def test_with_remote_configuration(self, project, init_local_table):
        ...
        _, log_output = run_dbt_and_capture(
            ["run", "--vars", json.dumps(materialized_var | local_suffix_var)]
        )
    ...
    def _assert_is_distributed_table(project):
        ...
        result = project.run_sql(
            f"select engine from system.tables where name='remote_table'", fetch="one"
        )
    ...
    def _assert_correct_engine(project):
        ...
        result = project.run_sql(
            f"select create_table_query from system.tables where name='remote_table'", fetch="one"
        )
        assert (
            f"Distributed('test_shard', '_{project.test_schema}', 'remote_table_local', key1)"
            in result[0]
        )
    ...
    def init_local_table(self, project):
        ...
        with get_connection(project.adapter):
            relation = project.adapter.Relation.create(
                database=project.database, schema=schema_name
            )
